...
//      permisos:= true;
//  end;
  if (Ansipos('0', sucursales) <> 0) or ((Ansipos(inttostr(suc_prod), sucursales) <> 0)) then
    permisos:= true;
                     
  if not (permisos and dm.ISUsrLogin.PermisoAccion('MODIFICAR_STOCK_AJEN')) then
  begin
...
begin
  ...
  if dm.ISUsrLogin.PermisoAccion('MODIFICAR_STOCK') then
  begin
  ...
  if dm.ISUsrLogin.PermisoAccion('ASOCIAR_STOCK_SUC') then
  begin
