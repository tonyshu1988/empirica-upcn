object FCompararStock: TFCompararStock
  Left = 301
  Top = 130
  Width = 870
  Height = 581
  Caption = 'Comparar Stock'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Verdana'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 862
    Height = 30
    Align = alTop
    TabOrder = 0
    object lblOrigen: TLabel
      Left = 1
      Top = 1
      Width = 860
      Height = 28
      Align = alClient
      Caption = ' lblOrigen'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -20
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 30
    Width = 862
    Height = 30
    Align = alTop
    TabOrder = 2
    object lblDestino: TLabel
      Left = 1
      Top = 1
      Width = 860
      Height = 28
      Align = alClient
      Caption = ' lblDestino'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -20
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 60
    Width = 862
    Height = 392
    Align = alClient
    DataSource = DataSource1
    Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 5
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Verdana'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'ID_STOCK_PRODUCTO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ID_PRODUCTO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ID_POSICION_SUCURSAL'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'STOCK_ACTUAL'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'STOCK_MIN'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'STOCK_MAX'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'STOCK_REPEDIDO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'STOCK_MIN_ALARMA'
        Visible = True
      end>
  end
  object Panel3: TPanel
    Left = 0
    Top = 504
    Width = 862
    Height = 50
    Align = alBottom
    TabOrder = 7
    DesignSize = (
      862
      50)
    object Label1: TLabel
      Left = 6
      Top = 5
      Width = 56
      Height = 13
      Caption = 'Progreso:'
    end
    object Label2: TLabel
      Left = 29
      Top = 28
      Width = 33
      Height = 13
      Caption = 'Total:'
    end
    object lblTotal: TLabel
      Left = 66
      Top = 28
      Width = 7
      Height = 13
      Caption = '0'
    end
    object Label4: TLabel
      Left = 113
      Top = 28
      Width = 93
      Height = 13
      Caption = '/   Actualizados:'
    end
    object lblActualizado: TLabel
      Left = 210
      Top = 28
      Width = 7
      Height = 13
      Caption = '0'
    end
    object ProgressBar1: TProgressBar
      Left = 66
      Top = 4
      Width = 780
      Height = 17
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 0
    end
  end
  object Conexion_Origen: TZConnection
    Protocol = 'firebird-1.5'
    HostName = '127.0.0.1'
    Database = 'C:\SISTEMAS\SiGeFa\Bases\SIGEFA LOPEZ.FDB'
    User = 'SYSDBA'
    Password = 'masterkey'
    AutoCommit = False
    Left = 40
    Top = 72
  end
  object Conexion_Destino: TZConnection
    Protocol = 'firebird-1.5'
    HostName = '127.0.0.1'
    Database = 'C:\SISTEMAS\SiGeFa\Bases\SIGEFA LOPEZ.FDB'
    User = 'SYSDBA'
    Password = 'masterkey'
    AutoCommit = False
    Left = 32
    Top = 128
  end
  object dxBarABM: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Backgrounds.Bar.Data = {
      
      8DE0A48DE0A48DE0A48D}
    Bars = <
      item
        AllowClose = False
        AllowCustomizing = False
        AllowQuickCustomizing = False
        BorderStyle = bbsNone
        Caption = 'barra'
        DockedDockingStyle = dsBottom
        DockedLeft = 0
        DockedTop = 0
        DockingStyle = dsBottom
        FloatLeft = 276
        FloatTop = 216
        FloatClientWidth = 39
        FloatClientHeight = 38
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ItemLinks = <
          item
            Item = btnBuscarOrigen
            Visible = True
          end
          item
            Item = btnBuscarDestino
            Visible = True
          end
          item
            Item = btnComparar
            Visible = True
          end
          item
            BeginGroup = True
            Item = btnSalir
            Visible = True
          end>
        Name = 'barra'
        OneOnRow = True
        Row = 0
        UseOwnFont = True
        UseRecentItems = False
        UseRestSpace = True
        Visible = True
        WholeRow = False
        BackgroundBitmap.Data = {
          }
      end>
    CanCustomize = False
    Categories.Strings = (
      'Edicion')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    HideFloatingBarsWhenInactive = False
    Images = FPrincipal.Iconos_Barra_32
    LargeImages = FPrincipal.Iconos_Barra_32
    LookAndFeel.Kind = lfUltraFlat
    LookAndFeel.NativeStyle = False
    MenusShowRecentItemsFirst = False
    NotDocking = [dsNone, dsLeft, dsTop, dsRight]
    PopupMenuLinks = <>
    ShowShortCutInHint = True
    Style = bmsOffice11
    UseF10ForMenu = False
    UseSystemFont = False
    Left = 32
    Top = 184
    DockControlHeights = (
      0
      0
      0
      52)
    object btnBuscarOrigen: TdxBarLargeButton
      Caption = 'Base Origen'
      Category = 0
      Hint = 'Base Origen'
      Visible = ivAlways
      ImageIndex = 29
      OnClick = btnBuscarOrigenClick
      AutoGrayScale = False
    end
    object btnBuscarDestino: TdxBarLargeButton
      Align = iaRight
      Caption = 'Base Destino'
      Category = 0
      Hint = 'Base Destino'
      Visible = ivAlways
      ImageIndex = 29
      OnClick = btnBuscarDestinoClick
      AutoGrayScale = False
    end
    object btnComparar: TdxBarLargeButton
      Caption = 'Comparar'
      Category = 0
      Hint = 'Comparar'
      Visible = ivAlways
      ImageIndex = 12
      OnClick = btnCompararClick
      AutoGrayScale = False
    end
    object btnGuardar: TdxBarLargeButton
      Caption = 'Guardar'
      Category = 0
      Enabled = False
      Hint = 'Guarda los cambios'
      Visible = ivAlways
      ImageIndex = 3
      AutoGrayScale = False
    end
    object btnCancelar: TdxBarLargeButton
      Caption = 'Cancelar'
      Category = 0
      Enabled = False
      Hint = 'Cancela los cambios'
      Visible = ivAlways
      ImageIndex = 4
      AutoGrayScale = False
    end
    object btnSalir: TdxBarLargeButton
      Align = iaRight
      Caption = 'Salir'
      Category = 0
      Hint = 'Salir sin seleccionar'
      Visible = ivAlways
      ImageIndex = 6
      OnClick = btnSalirClick
      AutoGrayScale = False
    end
    object GrupoEditando: TdxBarGroup
      Items = (
        'btnComparar'
        'btnBuscarOrigen'
        'btnBuscarDestino'
        'btnSalir')
    end
    object GrupoGuardarCancelar: TdxBarGroup
      Enabled = False
      Items = (
        'btnGuardar'
        'btnCancelar')
    end
  end
  object OpenFileOrigen: TOpenDialog
    DefaultExt = 'FDB'
    Filter = 'Base de Datos|*.FDB'
    InitialDir = 'c:\'
    Title = 'Base de datos'
    Left = 32
    Top = 232
  end
  object OpenFileDestino: TOpenDialog
    DefaultExt = 'FDB'
    Filter = 'Base de Datos|*.FDB'
    InitialDir = 'c:\'
    Title = 'Base de datos'
    Left = 32
    Top = 280
  end
  object ZQ_StockOrigen: TZQuery
    Connection = Conexion_Origen
    SQL.Strings = (
      'select sp.*'
      'from stock_producto sp'
      
        'left join posicion_sucursal ps on (sp.id_posicion_sucursal = ps.' +
        'id_posicion_sucursal)'
      'left join configuracion c on (c.id_sucursal = ps.id_sucursal)'
      'where (c.id_sucursal = ps.id_sucursal)')
    Params = <>
    Left = 152
    Top = 72
    object ZQ_StockOrigenID_STOCK_PRODUCTO: TIntegerField
      FieldName = 'ID_STOCK_PRODUCTO'
    end
    object ZQ_StockOrigenID_PRODUCTO: TIntegerField
      FieldName = 'ID_PRODUCTO'
    end
    object ZQ_StockOrigenID_POSICION_SUCURSAL: TIntegerField
      FieldName = 'ID_POSICION_SUCURSAL'
    end
    object ZQ_StockOrigenSTOCK_ACTUAL: TFloatField
      FieldName = 'STOCK_ACTUAL'
    end
    object ZQ_StockOrigenSTOCK_MIN: TFloatField
      FieldName = 'STOCK_MIN'
    end
    object ZQ_StockOrigenSTOCK_MAX: TFloatField
      FieldName = 'STOCK_MAX'
    end
    object ZQ_StockOrigenSTOCK_REPEDIDO: TFloatField
      FieldName = 'STOCK_REPEDIDO'
    end
    object ZQ_StockOrigenSTOCK_MIN_ALARMA: TStringField
      FieldName = 'STOCK_MIN_ALARMA'
      Size = 1
    end
  end
  object ZQ_VerSiCambio: TZQuery
    Connection = Conexion_Destino
    SQL.Strings = (
      'select sp.*'
      'from stock_producto sp'
      'where sp.id_producto = :id_producto'
      '  and sp.id_posicion_sucursal = :id_posicion'
      '  and sp.stock_actual = :stock_actual'
      '  and sp.stock_min = :stock_min'
      '  and sp.stock_max = :stock_max'
      '  and sp.stock_repedido = :stock_repedido'
      '  and sp.stock_min_alarma = :stock_min_alarma')
    Params = <
      item
        DataType = ftUnknown
        Name = 'id_producto'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_posicion'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_actual'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_min'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_max'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_repedido'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_min_alarma'
        ParamType = ptUnknown
      end>
    Left = 152
    Top = 184
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id_producto'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_posicion'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_actual'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_min'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_max'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_repedido'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_min_alarma'
        ParamType = ptUnknown
      end>
    object ZQ_VerSiCambioID_STOCK_PRODUCTO: TIntegerField
      FieldName = 'ID_STOCK_PRODUCTO'
    end
    object ZQ_VerSiCambioID_PRODUCTO: TIntegerField
      FieldName = 'ID_PRODUCTO'
    end
    object ZQ_VerSiCambioID_POSICION_SUCURSAL: TIntegerField
      FieldName = 'ID_POSICION_SUCURSAL'
    end
    object ZQ_VerSiCambioSTOCK_ACTUAL: TFloatField
      FieldName = 'STOCK_ACTUAL'
    end
    object ZQ_VerSiCambioSTOCK_MIN: TFloatField
      FieldName = 'STOCK_MIN'
    end
    object ZQ_VerSiCambioSTOCK_MAX: TFloatField
      FieldName = 'STOCK_MAX'
    end
    object ZQ_VerSiCambioSTOCK_REPEDIDO: TFloatField
      FieldName = 'STOCK_REPEDIDO'
    end
    object ZQ_VerSiCambioSTOCK_MIN_ALARMA: TStringField
      FieldName = 'STOCK_MIN_ALARMA'
      Size = 1
    end
  end
  object ZQ_Update: TZQuery
    Connection = Conexion_Destino
    SQL.Strings = (
      'update stock_producto'
      'set stock_actual = :stock_actual,'
      '    stock_min = :stock_min,'
      '    stock_max = :stock_max,'
      '    stock_repedido = :stock_repedido,'
      '    stock_min_alarma = :stock_min_alarma'
      'where id_producto = :id_producto'
      '    and id_posicion_sucursal = :id_posicion')
    Params = <
      item
        DataType = ftUnknown
        Name = 'stock_actual'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_min'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_max'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_repedido'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_min_alarma'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_producto'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_posicion'
        ParamType = ptUnknown
      end>
    Left = 152
    Top = 240
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'stock_actual'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_min'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_max'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_repedido'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'stock_min_alarma'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_producto'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_posicion'
        ParamType = ptUnknown
      end>
  end
  object DataSource1: TDataSource
    DataSet = ZQ_StockOrigen
    Left = 152
    Top = 120
  end
end
