...
procedure TFABM_CPB_FacturaCompra.FormCloseQuery(Sender: TObject;
...
procedure TFABM_CPB_FacturaCompra.configFormaPago(tipo: integer; edicion: boolean);
begin
  case tipo of
...
procedure TFABM_CPB_FacturaCompra.configDetalleProducto(tipo: integer);
begin
  case tipo of
...
procedure TFABM_CPB_FacturaCompra.mostrarNumeroCpbBase(flag: boolean);
begin
  if flag then
  begin
...
procedure TFABM_CPB_FacturaCompra.btnTipoCpb_AceptarClick(Sender: TObject);
begin
...
procedure TFABM_CPB_FacturaCompra.btnTipoCpb_CancelarClick(Sender: TObject);
begin
  PanelTipoCpb.Visible:= false;
  modoEdicion(false);
end;
...
procedure TFABM_CPB_FacturaCompra.btnReactivarClick(Sender: TObject);
var
  recNo: integer;
begin
...
procedure TFABM_CPB_FacturaCompra.configVisualizacion(); //configuro la pantalla de visualizacion segun el tipo de comprobante
begin
...
procedure TFABM_CPB_FacturaCompra.ssDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure TFABM_CPB_FacturaCompra.sDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure TFABM_CPB_FacturaCompra.onSelPersona;
begin
...
procedure TFABM_CPB_FacturaCompra.btnBuscarPersonaClick(Sender: TObject);
begin
...
//    FORMA DE PAGO
//----------------------
procedure TFABM_CPB_FacturaCompra.DBGridEditar_FpagoColExit(Sender: TObject);
begin
...
procedure TFABM_CPB_FacturaCompra.DBGridEditar_FpagoKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TFABM_CPB_FacturaCompra.btnEliminarFPagoClick(Sender: TObject);
begin
  if dm.EKModelo.verificar_transaccion(transaccion_ABM) then
  begin
    if not ZQ_CpbFormaPago.IsEmpty then
...
procedure TFABM_CPB_FacturaCompra.DBGridEditar_FpagoKeyPress(Sender: TObject; var Key: Char);
var
  columna: string;
begin
...
procedure TFABM_CPB_FacturaCompra.EKSuma_FPagoSumListChanged(Sender: TObject);
begin
...
procedure TFABM_CPB_FacturaCompra.mostrarPanelCompleto(panel: Tpanel; valor: boolean);
begin
...
procedure TFABM_CPB_FacturaCompra.PopUpItem_FPagoAgrandarClick(Sender: TObject);
begin
  if agrandarPanelFPago = false then
  begin
    agrandarPanelFPago:= true;
...
procedure TFABM_CPB_FacturaCompra.PopUpItem_ProductoAgrandarClick(Sender: TObject);
begin
  if agrandarPanelProducto = false then
  begin
    agrandarPanelProducto:= true;
...
procedure TFABM_CPB_FacturaCompra.AplicarFiltro(Sender: TObject);
begin
...
procedure TFABM_CPB_FacturaCompra.confirmarCpb(tipo: integer);
begin
  case tipo of
    CPB_NOTA_PEDIDO:  begin //CPB_NOTA_PEDIDO; COMPROBANTE_DETALLE
                        if confirmarComprobante then
...
procedure TFABM_CPB_FacturaCompra.ZQ_CpbProductoCANTIDAD_RECIBIDAChange(Sender: TField);
begin
...
procedure TFABM_CPB_FacturaCompra.DBGridEditar_ProductoDrawColumnCell(
