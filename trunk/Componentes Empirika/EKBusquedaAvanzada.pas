...
type
  ...
  TEKCriterioTipo = (EK_Texto, EK_Fecha, EK_Numero, EK_Blob, EK_Texto_Ordenado);
...
{$ENDIF}
    ...
    function armarnumero(x: integer; operador: TComboBox; campo: TEdit): string;
    function armartexto(x: integer; operador: TComboBox; campo: TEdit): string;
    function armartextoordenado(x: integer; operador: TComboBox; campo: TEdit): string;
...
//  else
 //   FMascara := '';
  ...
  if ((FTipoCampo = EK_Numero) or (FTipoCampo = EK_Fecha) or (FTipoCampo = EK_Texto_Ordenado)) then
    case FTipoCampoIndice of
      ...
      6: FTipoCampoIndiceTxt:= 'Es Nulo';
      7: FTipoCampoIndiceTxt:= 'Es No Nulo';
    else
      FTipoCampoIndicetxt:= '=';
      FTipoCampoIndice:= 0;
    end;
  ...
  if ((FTipoCampoIndice = 6) and ((FTipoCampo = EK_Numero) or (FTipoCampo = EK_Fecha) or (FTipoCampo = EK_Texto_Ordenado))) then
  begin
    FBacCondicion1:= 2;
    FBacCondicion2:= 4;
  end;
...
//        if (FTextoCombo2.Items[FTextoCombo2.ItemIndex] > '') then
    ...
    FparametrosSelecReales1.Add(textovalor3.text);
      ...
      //si ingrese texto o el combo dice ES NULO
    if (textovalor1.Text > '') or (FCondicion1.Items[FCondicion1.ItemIndex] = 'Es Nulo') or (FCondicion1.Items[FCondicion1.ItemIndex] = 'Es No Nulo') then
    begin
      if TipoCampo = EK_Texto then
      begin
      ...
      begin
        txt:= txt + ' AND ((' + armarnumero(x, FCondicion1, textovalor1) + ')';
        if (textovalor2.Text > '') then
          txt:= txt + ' ' + convertir_y_o(Fyo) + ' (' + armarnumero(x, FCondicion2, textovalor2) + ')';
      end;

      if TipoCampo = EK_Texto_Ordenado then
      begin
        txt:= txt + ' AND ((' + armartextoordenado(x, FCondicion1, textovalor1) + ')';
        if (textovalor2.Text > '') then
          txt:= txt + ' ' + convertir_y_o(Fyo) + ' (' + armartextoordenado(x, FCondicion2, textovalor2) + ')';
      end;
...
function TEKBusquedaAvanzada.armartexto(x: integer; operador: TComboBox; campo: TEdit): string;
var
  txt_campo: string;
  txt_condicion: string;
begin
  ...
  if txt_condicion = 'Igual' then
    armartexto:= ' UPPER(' + txt_campo + ') = ''' + UpperCase(campo.Text) + '''';
  if txt_condicion = 'Distinto' then
    armartexto:= ' UPPER(' + txt_campo + ') <> ''' + UpperCase(campo.Text) + '''';
  if txt_condicion = 'Empieza' then
    armartexto:= ' UPPER(' + txt_campo + ') LIKE ''' + UpperCase(campo.Text) + '%''';
  if txt_condicion = 'Termina' then
    armartexto:= ' UPPER(' + txt_campo + ') LIKE ''%' + UpperCase(campo.Text) + '''';
  if txt_condicion = 'Contiene' then
    armartexto:= ' UPPER(' + txt_campo + ') LIKE ''%' + UpperCase(campo.Text) + '%''';
  if txt_condicion = 'No Contiene' then
    armartexto:= ' UPPER(' + txt_campo + ') NOT LIKE ''%' + UpperCase(campo.Text) + '%''';
  if txt_condicion = 'Es Nulo' then
    armartexto:= ' UPPER(' + txt_campo + ') IS NULL';
  if txt_condicion = 'Es No Nulo' then
    armartexto:= ' UPPER(' + txt_campo + ') IS NOT NULL';
end;


function TEKBusquedaAvanzada.armartextoordenado(x: integer; operador: TComboBox; campo: TEdit): string;
var
  txt_campo: string;
  txt_condicion: string;
begin
  ...
  if txt_condicion = '=' then
    armartextoordenado:= ' UPPER(' + txt_campo + ') = ''' + UpperCase(campo.Text) + '''';
  if txt_condicion = '>=' then
    armartextoordenado:= ' UPPER(' + txt_campo + ') >= ''' + UpperCase(campo.Text) + '''';
  if txt_condicion = '<=' then
    armartextoordenado:= ' UPPER(' + txt_campo + ') <= ''' + UpperCase(campo.Text) + '''';
  ...
  if txt_condicion = '<>' then
    armartextoordenado:= ' UPPER(' + txt_campo + ') <> ''' + UpperCase(campo.Text) + '''';
  if txt_condicion = 'Es Nulo' then
    armartextoordenado:= ' UPPER(' + txt_campo + ') IS NULL';
  if txt_condicion = 'Es No Nulo' then
    armartextoordenado:= ' UPPER(' + txt_campo + ') IS NOT NULL';
end;


function TEKBusquedaAvanzada.armarnumero(x: integer; operador: TComboBox; campo: TEdit): string;
var
  txt_campo: string;
  txt_condicion: string;
begin
  ...
  if txt_condicion = '=' then
    armarnumero:= ' UPPER(' + txt_campo + ') = ''' + UpperCase(campo.Text) + '''';
  if txt_condicion = '>=' then
    armarnumero:= ' UPPER(' + txt_campo + ') >= ''' + UpperCase(campo.Text) + '''';
  if txt_condicion = '<=' then
    armarnumero:= ' UPPER(' + txt_campo + ') <= ''' + UpperCase(campo.Text) + '''';
  ...
  if txt_condicion = '<>' then
    armarnumero:= ' UPPER(' + txt_campo + ') <> ''' + UpperCase(campo.Text) + '''';
  if txt_condicion = 'Es Nulo' then
    armarnumero:= ' UPPER(' + txt_campo + ') IS NULL';
  if txt_condicion = 'Es No Nulo' then
    armarnumero:= ' UPPER(' + txt_campo + ') IS NOT NULL';
end;
