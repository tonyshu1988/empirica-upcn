...
uses
  Classes, Graphics, StdCtrls, ExtCtrls, StrUtils, Dialogs,
  QRCtrls, QuickRpt;
...
//
  ...
  TEKCodigoBarra = class(TComponent)
  private
...
// Additions from Roberto Parola to improve the text output
  ...
  published
   ...
   { Height of Barcode (Pixel)}
    ...
    property CTop    : Integer read FCTop write SetTop;
    property CLeft   : Integer read FCLeft write SetLeft;
   ...
   { 0 - 360 degree }
    ...
    {Agregados}
    property Texto : Tedit read FTexto write FTexto;
    property Codigo : TImage read FCodigo write FCodigo;
    property QRTexto : TQRLabel read FQRTexto write FQRTexto;
    property QRCodigo : TQRImage read FQRCodigo write FQRCodigo;

  end;

procedure Register;

function Devuelve_Digito_Ctrl(cadena:String): integer;
function Sumar_Digitos_Ctrl(numero:integer): integer;
...
constructor TEKCodigoBarra.Create(Owner:TComponent);
begin
...
destructor TEKCodigoBarra.Destroy;
begin
  FShowTextFont.Free;
  inherited;
end;

procedure TEKCodigoBarra.Assign(Source: TPersistent);
var
   BSource : TEKCodigoBarra;
begin
   if Source is TEKCodigoBarra then
   begin
      BSource    := TEKCodigoBarra(Source);
      FHeight    := BSource.FHeight;
      FText      := BSource.FText;
      FCTop       := BSource.FCTop;
      FCLeft      := BSource.FCLeft;
...
function TEKCodigoBarra.GetTypText:string;
begin
...
{ set Modul Width  }
procedure TEKCodigoBarra.SetModul(v:integer);
begin
...
procedure TEKCodigoBarra.OneBarProps(code:char; var Width:integer; var lt:TBarLineType);
begin
  case code of
...
function TEKCodigoBarra.MakeData : string;
var
  i : integer;
begin
...
function TEKCodigoBarra.GetWidth:integer;
var
  data : string;
  i : integer;
  w : integer;
...
procedure TEKCodigoBarra.SetWidth(Value :integer);
var
  data : string;
  i : integer;
  w, wtotal : integer;
...
function TEKCodigoBarra.DoCheckSumming(const data : string):string;
begin
  case FCheckSumMethod of
    ...
    csmDigitoCtrl:
      Result := inttostr(Devuelve_Digito_Ctrl(data));
  end;
end;
...
function TEKCodigoBarra.Code_EAN8:string;
var
  i : integer;
  tmp : String;
begin
...
function TEKCodigoBarra.Code_EAN13:string;
var
  i, LK: integer;
  tmp : String;
begin
...
function TEKCodigoBarra.Code_2_5_interleaved:string;
var
  i, j: integer;
  c : char;
  cod : string;
begin
  ...
  cod := FText;
  if Checksum and (CheckSumMethod = csmDigitoCtrl) then
    cod := cod + DoCheckSumming(FText);

  result := '5050';   {Startcode}

  for i:=1 to Length(cod) div 2 do
  begin
    for j:= 1 to 5 do
    begin
      if tabelle_2_5[cod[i*2-1], j] = '1' then
        c := '6'
      else
        c := '5';
      result := result + c;
      if tabelle_2_5[cod[i*2], j] = '1' then
        c := '1'
      else
        c := '0';
      result := result + c;
  ...
  if Assigned(FTexto) then
    FTexto.Text := cod;
  if Assigned(FQRTexto) then
    FQRTexto.Caption := cod;
end;


function TEKCodigoBarra.Code_2_5_industrial:string;
var
  i, j: integer;
begin
...
function TEKCodigoBarra.Code_2_5_matrix:string;
var
  i, j: integer;
  c :char;
begin
...
function TEKCodigoBarra.Code_39:string;
...
function TEKCodigoBarra.Code_39Extended:string;
...
function TEKCodigoBarra.Code_128:string;
type TCode128 =
  record
    a, b : char;
    c : string[2];
...
function TEKCodigoBarra.Code_93:string;
type TCode93 =
  record
    c : char;
...
function TEKCodigoBarra.Code_93Extended:string;
...
function TEKCodigoBarra.Code_MSI:string;
...
function TEKCodigoBarra.Code_PostNet:string;
...
function TEKCodigoBarra.Code_Codabar:string;
type TCodabar =
  record
    c : char;
...
{Assist function}
function TEKCodigoBarra.SetLen(pI:byte):string;
begin
...
function TEKCodigoBarra.Code_UPC_A:string;
var
  i : integer;
  tmp : String;
begin
...
function TEKCodigoBarra.Code_UPC_E0:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TEKCodigoBarra.Code_UPC_E1:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TEKCodigoBarra.Code_Supp5:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TEKCodigoBarra.Code_Supp2:string;
var i,j : integer;
   tmp,mS : String;
begin
  FText := SetLen(2);
...
procedure TEKCodigoBarra.MakeModules;
begin
  case Typ of
    bcCode_2_5_interleaved,
    bcCode_2_5_industrial,
...
procedure TEKCodigoBarra.DoLines(data:string; Canvas:TCanvas);
...
procedure TEKCodigoBarra.DrawBarcode(Canvas:TCanvas);
var
  data : string;
  SaveFont: TFont;
  SavePen: TPen;
...
procedure TEKCodigoBarra.DrawText(Canvas:TCanvas);
var
  PosX, PosY: Integer;
  SaveFont: TFont;
begin
...
// As stated by Andreas Schmidt
        ...
        if ShowTextPosition in [stpTopLeft, stpTopCenter, stpTopRight] then
          ...
          if ShowTextPosition in [stpBottomLeft, stpBottomCenter, stpBottomRight] then
            PosY := FCTop + Height - TextHeight(Text);

        if FShowText in [bcoCode, bcoBoth] then
//          TextOut(FCLeft, FCTop, FText);         {contents of Barcode}
          TextOut(PosX, PosY, FText);         {contents of Barcode}
        if FShowText in [bcoTyp, bcoBoth] then
          TextOut(FCLeft, FCTop+Round(Font.Height*2.5), GetTypText); {type/name of barcode}
      finally
...
procedure TEKCodigoBarra.DoChange;
var
  img : timage;
begin
   if Assigned(FTexto) then
   ...
   if Assigned(FQRTexto) then
      FQRTexto.Caption := FText;

   if Assigned(FCodigo) then
   begin
      FCodigo.Canvas.FillRect(FCodigo.Canvas.ClipRect);
      DrawBarcode(FCodigo.Canvas);
   end;

   if Assigned(FQRCodigo) then
   begin
    img := TImage.Create(nil);
    img.Height := FQRCodigo.Height;
    img.Width := FQRCodigo.Width;
    img.Canvas.FillRect(img.Canvas.ClipRect);
    DrawBarcode(img.Canvas);
    FQRCodigo.Picture.Assign(img.Picture);
    img.Free;
   end;
...
procedure TEKCodigoBarra.SetRatio(const Value: Double);
begin
...
procedure TEKCodigoBarra.SetTyp(const Value: TBarcodeType);
begin
   if Value <> FTyp then
   begin
      FTyp := Value;
...
procedure TEKCodigoBarra.SetAngle(const Value: Double);
begin
...
procedure TEKCodigoBarra.SetText(const Value: string);
begin
   if Value <> FText then
   begin
      FText := Value;
...
procedure TEKCodigoBarra.SetShowText(const Value: TBarcodeOption);
begin
...
procedure TEKCodigoBarra.SetTop(const Value: Integer);
begin
   if Value <> FCTop then
   begin
      FCTop := Value;
...
procedure TEKCodigoBarra.SetLeft(const Value: Integer);
begin
   if Value <> FCLeft then
   begin
      FCLeft := Value;
      DoChange;
   end;
...
procedure TEKCodigoBarra.SetCheckSum(const Value: Boolean);
begin
...
procedure TEKCodigoBarra.SetHeight(const Value: integer);
begin
...
procedure Register;
begin
  RegisterComponents('EmpiriKa', [TEKCodigoBarra]);
end;

function TEKCodigoBarra.GetCanvasHeight: Integer;
var
  alpha :Extended;
begin
  alpha := FAngle/180.0*pi;
...
function TEKCodigoBarra.GetCanvasWidth: Integer;
var
  alpha :Extended;
begin
  alpha := FAngle/180.0*pi;
...
procedure TEKCodigoBarra.SetShowTextFont(const Value: TFont);
begin
  FShowTextFont.Assign(Value);
  DoChange;
end;

procedure TEKCodigoBarra.SetShowTextPosition(const Value: TShowTextPosition);
begin
...
function Devuelve_Digito_Ctrl(cadena:String):integer;
// esta funcion recibe una cadena de digitos
//y luego a través de unos calculos obtiene el digito de control.
var
  i: byte;
  resul: integer;
  aux: string;
begin
     aux:=ReverseString(cadena);

     Resul:=0;
     for i:= 1 to Length(aux) do
       if i mod 2 <> 0 then
        resul:= resul + Sumar_Digitos_Ctrl( StrToint(aux[i])* 2)
       else
        resul:= resul +  StrToint(aux[i]);

     if resul mod 10 = 0 then
      resul:=0
     else
      resul := (((resul div 10) + 1) * 10) - resul;

     Devuelve_Digito_Ctrl:= resul; // devuelvo el valor encontrado
...
begin
 ...
 if Length(cadena)= 1 then
  Sumar_Digitos_Ctrl:=strtoint(cadena)
 else
  begin
   for i:=1 to length(cadena) do
    resultado := resultado + strtoint(cadena[i]);
   Sumar_Digitos_Ctrl:=Sumar_Digitos_Ctrl(resultado);
  end
end;
